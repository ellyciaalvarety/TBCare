//features/medis/patients/ajukan_jadwal_screen.dart
...
class _AjukanJadwalScreenState extends State<AjukanJadwalScreen> {
  ...
  // Fungsi konfirmasi untuk menyimpan data pengajuan jadwal ke SQLite
  Future<void> _konfirmasi() async {
    ...
    try {
      ...
      // 1. Ambil nama pasien dari tabel 'patients' berdasarkan patientId
      final List<Map<String, dynamic>> patientResult = await db.query(
        'patients',
        where: 'id = ?',
      ...
      if (patientResult.isNotEmpty) {
        patientName = patientResult.first['nama'] ?? 'Pasien Baru';
      }

      // 2. Format tanggal & waktu (Contoh hasil format: "24 Okt 2023, 08:00")
      final String formattedDate =
          "${_selectedDate!.day} ${_monthNames[_selectedDate!.month].substring(0, 3)} ${_selectedDate!.year}";
      final String timeRange =
          "$_selectedTime — ${int.parse(_selectedTime.split(':')[0]) + 1}:00";

      // 3. Insert ke dalam tabel 'appointments'
      await db.insert('appointments', {
        'id': DateTime.now().millisecondsSinceEpoch
            .toString(), // ID Unik berbasis timestamp
        'patientName': patientName,
        'patientId': widget.patientId,
        'type': _agendaCtrl.text.trim(),
        'time': '$formattedDate ($timeRange)',
        'room': _ruanganCtrl.text.trim(),
        'isCompleted': 0, // Default belum selesai (false)
      });
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _InputField(controller: _agendaCtrl, hint: 'Preview test'),
          const SizedBox(height: 16),

          // Ruangan
          _FieldLabel(label: 'Ruangan'),
          const SizedBox(height: 6),
          _InputField(controller: _ruanganCtrl, hint: 'A 3.06'),
          const SizedBox(height: 16),
